...
export default function RadioPlayer() {
  ...
  const { isEngaged, engagementScore, addInteraction } = useRadioEngagement();
  ...
  const STREAM_URL = 'https://icecast.vision.org.au/vision';

  useEffect(() => {
    if (audioRef.current) {
      audioRef.current.volume = isMuted ? 0 : volume;
      // Track volume changes
      addInteraction(); // Assuming volume change is an interaction
    }
  }, [volume, isMuted]);
