...
export async function GET(request: NextRequest) {
  try {
    ...
    const personalityTypes = psychographicProfiles.reduce((acc: any, p: any) => {
      acc[p.psychographic.personalityType] =
        (acc[p.psychographic.personalityType] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const messageFramingStats = psychographicProfiles.reduce((acc: any, p: any) => {
      acc[p.psychographic.messageFraming] =
        (acc[p.psychographic.messageFraming] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const copyToneStats = psychographicProfiles.reduce((acc: any, p: any) => {
    ...
    const highReadiness = psychographicProfiles.filter(
      (p: any) => p.psychographic.conversionReadiness >= 70
    ).length;
    ...
    const lowReadiness = psychographicProfiles.filter(
      (p: any) => p.psychographic.conversionReadiness < 40
    ).length;

    // Resistance levels
    const resistanceStats = psychographicProfiles.reduce((acc: any, p: any) => {
    ...
    // Urgency levels
    const urgencyStats = psychographicProfiles.reduce((acc: any, p: any) => {
    ...
    psychographicProfiles.forEach((p: any) => {
      p.psychographic.painPoints.forEach((point: string) => {
    ...
    psychographicProfiles.forEach((p: any) => {
      p.psychographic.motivators.forEach((motivator: string) => {
    ...
    const highValueTargets = psychographicProfiles
      ...
      .sort((a: any, b: any) => b.psychographic.conversionReadiness - a.psychographic.conversionReadiness)
      .slice(0, 20);

    // Sort profiles by conversion readiness
    psychographicProfiles.sort(
      (a: any, b: any) => b.psychographic.conversionReadiness - a.psychographic.conversionReadiness
    );
